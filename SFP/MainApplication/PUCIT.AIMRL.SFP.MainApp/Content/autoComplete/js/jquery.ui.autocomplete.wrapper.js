...
function JQUIAutoCompleteWrapper(options) {
    ...
    var defaults = {
        ...
        */
        ...
        maxItemsToShowInSearchResult:10
    };
    ...
    this.InitializeControl = function () {
        ...
        var $autoComplOpt = {
            ...
            source: function (request, response) {
                ...
                if (typeof opts.dataSource === 'function') {
                    ...
                    var dsource = opts.dataSource(request.term);
                    ...
                    for (var i = 0; i < dsource.length; i++) {
                        ...
                        if (result == false && opts.searchInValueField) {
                            result = matcher.test(item[opts.fields.ValueField]);
                        }
                        if (result) {
                            resultToReturn.push(item);
                            counter++;
                            if (counter == opts.maxItemsToShowInSearchResult)
                                break;
                        }
                    }

                    //var d = dsource.find(function (item) {
                    //    var result = matcher.test(item[opts.fields.DisplayField]);
        ...
        $autoCompObject.data("ui-autocomplete")._renderItem = function (ul, item) {
        ...
        $autoCompObject.data("ui-autocomplete")._resizeMenu = function () {
